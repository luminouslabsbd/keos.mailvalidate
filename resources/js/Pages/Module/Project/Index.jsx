...
function Index() {
    const { allProjectData, base_url } = usePage().props;

    const [isDeleteModal, setIsDeleteModal] = useState(false);
    const [deleteId, setDeleteId] = useState(false);
    const handleCreate = (id) => {
        router.get(`/admin/project/create`);
    };

    const handleEdit = (id) => {
        router.get(`/admin/project/${id}/edit`);
    };
    ...
    const handleCheckboxChange = async (id, checked) => {
        try {
            ...
            router.post("/admin/project/status", { id, status: updatedStatus });
        } catch (error) {
        ...
        {console.log(allProjectData)}
            <div className="pt-5">
                <div className="col-span-12 pt-4">
                    <div className="panel">
                        <div className="mb-2">
                            <h5 className="font-bold">Project list</h5>
                        </div>
                        <hr/>
                                    ...
                                    <tr key={project.id}>
                                        ...
                                        <td>
                                            <label className="w-12 h-6 relative">
                                                <input
                                                    ...
                                                    checked={project.status === 1} // Check if status is 1
                                                    onChange={(e) => handleCheckboxChange(project.id, e.target.checked)} // Handle checkbox change
                                                />
                                        ...
                                        <td className="!tex-right">
                                            <div className="flex justify-end">
                                                <a href="#" className="inline-block px-2 py-1 leading-none border border-blue-500 text-blue-500 rounded-md hover:text-white hover:bg-blue-500 mr-2" title="Edit" onClick={() => handleEdit(project.id)}>
                                                    <i className="las la-edit"></i>Edit
                                                </a>
                                                <a href="#" className="inline-block px-2 py-1 leading-none border border-red-500 text-red-500 rounded-md hover:text-white hover:bg-red-500 mr-2" title="Delete" onClick={() => handleDelete(project.id)}>
